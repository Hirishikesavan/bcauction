'use client';
import { User, Lock, TriangleAlert, Trash2 } from 'lucide-react';
...
export default function ProfilePage() {
  ...
  const changePassword = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await api.put('/user/change-password', { currentPassword: pwForm.current, newPassword: pwForm.newPw });
      ...
      <div className="min-h-screen bg-background">
        ...
        <div className="sticky top-0 z-20 bg-glass-navy" style={{ borderBottom: '1px solid hsla(45,100%,51%,0.12)' }}>
          <div className="max-w-3xl mx-auto px-6 py-4 flex items-center justify-between">
            ...
            <div className="flex items-center gap-3">
              ...
              {user?.role && (
                <Link href={getRoleRedirect(user.role) || '/select-role'} className="text-xs font-heading uppercase tracking-wider text-primary hover:bg-primary/10 px-3 py-2 rounded-lg border border-primary/30 transition-all">← Dashboard</Link>
              )}
        ...
        <div className="max-w-3xl mx-auto px-6 py-10">
          ...
          <div className="flex gap-2 mb-7">
            {([['profile', 'user', 'My Profile'], ['password', 'lock', 'Password'], ['danger', 'warning', 'Danger']] as [Tab, string, string][]).map(([id, ic, lb]) => (
          ...
          {tab === 'danger' && (
            <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}
              ...
              <h3 className="font-heading text-xl uppercase tracking-wider mb-2" style={{ color: 'hsl(0 84% 60%)' }}>Danger Zone</h3>
              ...
              <div className="p-5 rounded-lg" style={{ background: 'hsla(0,84%,60%,0.05)', border: '1px solid hsla(0,84%,60%,0.2)' }}>
                ...
                <button onClick={deleteAccount} disabled={loading || deleteConfirm !== user?.email}
                  ...
                  {loading ? 'Deleting...' : <><Trash2 size={14} className="inline mr-1.5" />Delete My Account</>}
                </button>
              </div>
            </motion.div>
          )}
