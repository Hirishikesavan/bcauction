'use client';
import { Gavel, Calendar, IndianRupee, Clock, Tag, Radio, Eye } from 'lucide-react';
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <div className="relative py-20 overflow-hidden">
        ...
        <div className="absolute inset-0" style={{ background:'linear-gradient(180deg,hsl(0 0% 0%/0.5) 0%,hsl(0 0% 0%) 100%)' }}/>
        <GoldParticles/>
                ...
                <motion.div key={a._id}
                  ...
                  <div className="p-5">
                    ...
                    <div className="space-y-1 text-xs font-display text-muted-foreground mb-4">
                      <div><Calendar size={11} className="inline mr-1 opacity-60" />{format(new Date(a.date), 'dd MMM yyyy, hh:mm a')}</div>
                      <div><IndianRupee size={11} className="inline mr-1 opacity-60" />{fmt(a.totalPursePerTeam)} per team · <Clock size={11} className="inline mr-1 opacity-60" />{a.bidTimer}s timer</div>
                      <div><Tag size={11} className="inline mr-1 opacity-60" />By {a.organizerId?.name || 'Organizer'}</div>
                    </div>
