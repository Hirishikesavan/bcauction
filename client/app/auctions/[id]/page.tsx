...
import {
  Gavel, Target, Trophy, Crown, Frown, Clapperboard, Star, Bot, Loader2,
  XCircle, Cast, AlertTriangle, Eye, ArrowLeft, Play, SkipForward,
  FlagTriangleRight, PauseCircle, Trophy as TrophyIcon,
} from 'lucide-react';
...
export default function LiveAuctionPage() {
  ...
  const isElite = user?.role === 'admin' || orgPackage?.packageType === 'elite';
  ...
  useEffect(() => {
    ...
    api.get(`/auctions/${id}/plan`).then(r => setOrgPackage(r.data)).catch(()=>{});
    // FIX: sponsors were only shown on broadcast/audience screens, never on
    // the main organizer/team-owner auction page itself.
  ...
  useEffect(() => {
    ...
    // FIX: rejoin on every reconnect, not just the initial mount — a
    // dropped connection (server restart, sleep/wake, brief network blip)
    // previously left this client "connected" but silently out of the
    // room, so live bids/players stopped updating with no visible error.
    const handleConnect = () => { setConnected(true); joinRoom(); };
    ...
    socket.on('auctionResumed', ()=>{ setAuctionStatus('active'); toast('Resumed', { icon: '▶' }); });
    ...
    socket.on('playerSold', async (d:any)=>{
      ...
      if (d.commentary) setAiCommentary(d.commentary);

      // Log activity to admin dashboard - use soldTo instead of team
      await logActivityToAdmin('player_sold', `Player sold: ${d.player.name} to ${d.soldTo?.teamName || 'a team'} for ₹${fmt(d.soldPrice)}`, {
        playerId: d.player._id,
        playerName: d.player.name,
        teamId: d.soldTo?.teamId,
        teamName: d.soldTo?.teamName,
    ...
    socket.on('auctionCompleted',()=>{
      setAuctionStatus('completed'); setCurrentPlayer(null);
      toast.success('Auction Completed Successfully!', { duration: 4000 });
      // Auto-redirect to appropriate dashboard after 4 seconds
      setTimeout(() => {
        const role = typeof window !== 'undefined' ? localStorage.getItem('pending_role') || '' : '';
  ...
  return (
    <div className="h-screen flex flex-col overflow-hidden" style={{background:'hsl(0 0% 0%)'}}>
          ...
          {isOrganizer && (
            <div className="flex gap-1.5 mr-2">
              ...
              {auctionStatus==='paused' && (
                <button onClick={()=>emit('resumeAuction')} className="px-3 py-1.5 rounded-lg text-xs font-bold transition-all text-black inline-flex items-center gap-1" style={{background:'hsl(142,70%,45%)'}}><Play size={12} aria-hidden="true" /> Resume</button>
              )}
              {currentPlayer && auctionStatus==='active' && (
                <button onClick={()=>emit('skipPlayer')} className="px-3 py-1.5 rounded-lg text-xs font-bold transition-all text-foreground inline-flex items-center gap-1" style={{background:'hsl(0 0% 10%)'}}><SkipForward size={12} aria-hidden="true" /> Skip</button>
              )}
              {currentPlayer && auctionStatus==='active' && leadingTeamId && (
                <button onClick={()=>emit('forceSell')} className="px-3 py-1.5 rounded-lg text-xs font-bold transition-all text-white inline-flex items-center gap-1" style={{background:'hsl(0,72%,51%)'}}><Gavel size={12} aria-hidden="true" /> Sell</button>
              )}
              {(auctionStatus==='active'||auctionStatus==='paused') && (
                <button
                  onClick={()=>{
                    if (window.confirm('End the auction now? All remaining players will be marked unsold. This cannot be undone.')) {
                      emit('endAuction');
                    }
                  }}
                  className="px-3 py-1.5 rounded-lg text-xs font-bold transition-all text-white inline-flex items-center gap-1"
                  style={{background:'hsl(280,60%,40%)', border:'1px solid hsl(280,60%,55%)'}}>
                  <FlagTriangleRight size={14} className="inline mr-1.5" />End Auction
                </button>
              )}
            </div>
          )}
      ...
      {/* SPONSOR STRIP — visible to everyone, large & clear */}
      {sponsors.length > 0 && (
        <div className="flex-shrink-0 border-b px-4 py-3 flex items-center justify-center gap-6 flex-wrap"
          style={{ borderColor:'hsla(45,100%,51%,0.25)', background: 'linear-gradient(90deg, hsla(222,47%,7%,0.6), hsla(45,60%,15%,0.25), hsla(222,47%,7%,0.6))' }}>
          <span className="text-xs font-heading uppercase tracking-[0.2em] text-amber-400 font-bold inline-flex items-center gap-1.5"><Star size={12} aria-hidden="true" /> Sponsored By <Star size={12} aria-hidden="true" /></span>
          {sponsors.map((s: any) => (
            <a key={s._id} href={s.websiteUrl || '#'} target="_blank" rel="noopener noreferrer"
              className="bg-white rounded-lg px-3 py-1.5 transition-transform hover:scale-105"
              style={{ border: '1.5px solid hsl(45,100%,51%)', boxShadow: '0 0 10px hsla(45,100%,51%,0.3)' }}>
              {s.logoUrl ? <img src={imgUrl(s.logoUrl)} alt={s.name} className="h-10 max-w-[160px] object-contain" /> : <span className="text-xs font-bold text-gray-800 px-1">{s.name}</span>}
            </a>
          ))}
        </div>
      )}
      ...
      <AnimatePresence>
        {rtmWindowOpen && soldData && (
          <motion.div initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}}
            ...
            style={{background:'hsl(0 0% 0%)',backdropFilter:'blur(12px)'}}>
            <motion.div initial={{scale:0.8,y:30}} animate={{scale:1,y:0}} transition={{type:'spring',stiffness:300,damping:25}}
              ...
              style={{background:'linear-gradient(180deg,hsl(0 0% 5%),hsl(0 0% 0%))',border:'2px solid hsla(45,100%,51%,0.5)',boxShadow:'0 0 80px hsla(45,100%,51%,0.2)'}}>
              <div style={{background:'linear-gradient(135deg,hsl(45,100%,51%),hsl(40,100%,38%))',padding:'20px',textAlign:'center'}}>
                <h2 style={{fontFamily:'Oswald,sans-serif',fontSize:'34px',color:'#000',margin:0,letterSpacing:'2px'}} className="flex items-center gap-2"><Target size={30} aria-hidden="true" /> RIGHT TO MATCH</h2>
                ...
                {isTeamOwner && myTeam && rtmRemaining>0 && !rtmUsed && soldData.soldTo?.teamId!==myTeamId ? (
                  <div className="space-y-3">
                    ...
                    <motion.button onClick={triggerRTM} whileHover={{scale:1.02}} whileTap={{scale:0.97}}
                      ...
                      <span className="relative z-10 inline-flex items-center gap-1.5"><Target size={14} aria-hidden="true" /> USE RTM — MATCH {soldData.soldPriceFormatted}</span>
                    ...
                    <button onClick={declineRTM} className="w-full py-3 rounded-xl text-muted-foreground hover:text-foreground text-sm font-bold transition-all inline-flex items-center justify-center gap-1.5" style={{border:'1px solid rgba(255,255,255,0.1)'}}><XCircle size={14} aria-hidden="true" /> Decline</button>
                  </div>
                ) : (
                  <div className="space-y-3">
                    <div className="rounded-xl p-4 text-center" style={{background:'rgba(255,255,255,0.04)',border:'1px solid rgba(255,255,255,0.08)'}}>
                      {soldData.soldTo?.teamId===myTeamId ? <p className="text-green-400 font-bold flex items-center gap-1.5"><Trophy size={14} aria-hidden="true" /> You won this player!</p>
      ...
      <div className="flex flex-1 overflow-hidden">
              ...
              return (
                <div key={team._id} className="rounded-xl p-3 border transition-all duration-300"
                  ...
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    {leading&&<Crown size={14} className="flex-shrink-0 text-yellow-400" aria-hidden="true" />}
                  </div>
                  ...
                  <div className="flex justify-between mt-1" style={{fontSize:'10px'}}>
                    ...
                    {(team.rtmTotal-team.rtmUsed)>0&&<span style={{color:'hsl(45,100%,51%)',fontWeight:700}} className="inline-flex items-center gap-0.5"><Target size={11} aria-hidden="true" />{team.rtmTotal-team.rtmUsed}</span>}
                  </div>
                </div>
              );
            })}
        ...
        <div className="flex-1 flex flex-col items-center justify-center relative overflow-hidden">
          ...
          <div className="absolute inset-0" style={{background:'radial-gradient(ellipse 80% 80% at 50% 50%, hsla(0,0%,5%,0.3) 0%, hsl(0,0%,0%) 70%)'}}/>
          {/* Subtle grid lines */}
          ...
          <AnimatePresence>
            {soldData&&!rtmWindowOpen&&(
              <motion.div initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}}
                ...
                style={{background:'hsl(0 0% 0%)',backdropFilter:'blur(6px)'}}>
                <div className="text-center px-8">
                  <motion.div initial={{scale:0,rotate:-20}} animate={{scale:1,rotate:0}} transition={{type:'spring',stiffness:300,damping:15}} style={{marginBottom:'12px'}} className="flex justify-center"><Gavel size={72} className="text-yellow-400" aria-hidden="true" /></motion.div>
            ...
            {unsoldPlayer&&!soldData&&(
              <motion.div initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}}
                className="absolute inset-0 z-20 flex items-center justify-center"
                style={{background:'hsl(0 0% 0%)',backdropFilter:'blur(4px)'}}>
                <div className="text-center">
                  <Frown size={56} className="mx-auto text-muted-foreground" style={{marginBottom:'12px'}} aria-hidden="true" />
          ...
          {(auctionStatus==='draft'||auctionStatus==='idle')&&!isOrganizer&&(
            <div className="text-center px-6 relative z-10">
              <motion.div animate={{scale:[1,1.04,1]}} transition={{duration:2.5,repeat:Infinity}} style={{marginBottom:'20px'}} className="flex justify-center"><Gavel size={64} className="text-muted-foreground" aria-hidden="true" /></motion.div>
          ...
          {(auctionStatus==='draft'||auctionStatus==='idle')&&isOrganizer&&(
            <div className="text-center px-6 relative z-10">
              <Clapperboard size={56} className="mx-auto text-primary" style={{marginBottom:'20px'}} aria-hidden="true" />
              ...
              <button onClick={()=>emit('startAuction')} className="px-10 py-4 rounded-xl font-bold text-black text-lg transition-all hover:scale-[1.04] inline-flex items-center gap-2" style={{fontFamily:'Oswald,sans-serif',background:'linear-gradient(135deg,hsl(45,100%,51%),hsl(40,100%,38%))',boxShadow:'0 0 40px hsla(45,100%,51%,0.4)',letterSpacing:'2px'}}>
                <Play size={20} aria-hidden="true" /> START AUCTION
              </button>
            </div>
          )}
          {auctionStatus==='completed'&&(
            <div className="text-center px-6 relative z-10">
              <Trophy size={64} className="mx-auto text-primary animate-bounce" style={{marginBottom:'12px'}} aria-hidden="true" />
              <h2 className="text-gradient-gold" style={{fontFamily:'Oswald,sans-serif',fontSize:'52px'}}>AUCTION COMPLETED SUCCESSFULLY!</h2>
              <p className="text-muted-foreground mt-2 mb-2" style={{fontFamily:'Rajdhani,sans-serif'}}>All players have been auctioned. Final results saved.</p>
              <p className="text-primary/70 text-sm mb-6 font-display">Redirecting to your dashboard in a few seconds…</p>
              <div className="flex gap-3 justify-center flex-wrap">
                <Link href="/auctions" className="inline-block px-6 py-3 rounded-xl font-bold text-muted-foreground border border-border transition-all hover:scale-[1.02]" style={{fontFamily:'Oswald,sans-serif'}}>View All Auctions</Link>
                <Link href="/dashboard/team-owner" className="inline-block px-6 py-3 rounded-xl font-bold text-black transition-all hover:scale-[1.04]" style={{fontFamily:'Oswald,sans-serif',background:'linear-gradient(135deg,hsl(45,100%,51%),hsl(40,100%,38%))'}}>Go to Dashboard</Link>
              </div>
            </div>
          )}
              ...
              <div className="relative flex gap-5 items-end w-full justify-center">
                ...
                <motion.div key={currentPlayer._id} initial={{scale:0.88,opacity:0,y:20}} animate={{scale:1,opacity:1,y:0}} transition={{type:'spring',stiffness:200,damping:20}}
                  ...
                  style={{width:'175px',height:'235px',
                    boxShadow:`0 0 0 2px hsla(45,100%,51%,0.35), 0 0 60px hsla(45,100%,51%,0.2), 0 20px 60px hsl(0 0% 0%)`,
                    background:'linear-gradient(180deg,hsl(0 0% 5%),hsl(0 0% 0%))',
                  }}>
                  {currentPlayer.imageUrl
                    ?<img src={imgUrl(currentPlayer.imageUrl)} alt={currentPlayer.name} className="w-full h-full object-cover object-top"/>
                    :<div className="w-full h-full flex items-center justify-center" style={{background:'linear-gradient(135deg,hsl(0 0% 8%),hsl(0 0% 5%))'}}>
                ...
                <div className="flex flex-col gap-2 min-w-[120px]">
                  ...
                  <div className={`text-center rounded-xl p-3 transition-all ${bidFlash?'scale-105':''}`}
                    ...
                    {leadingTeamId&&<div className="font-bold mt-1 truncate flex items-center gap-1" style={{fontFamily:"Rajdhani,sans-serif",fontSize:"12px",color:leadingTeamColor}}><Crown size={11} aria-hidden="true" /> {leadingTeamName}</div>}
                  </div>
                  {/* Stats */}
        ...
        <div className="w-64 xl:w-72 flex-shrink-0 border-l border-border/40 flex flex-col" style={{background:'hsla(222,42%,7%,0.97)'}}>
          <div className="p-3 border-b border-border/40 space-y-3">
            ...
            {isOrganizer&&(
              <>
                <div className="rounded-xl p-4 text-center" style={{background:'hsla(45,100%,51%,0.06)',border:'1px solid hsla(45,100%,51%,0.2)'}}>
                  <Clapperboard size={24} className="mx-auto text-primary" style={{marginBottom:"8px"}} aria-hidden="true" />
                ...
                {/* AI Commentary (Elite) */}
                {isElite && aiCommentary && (
                  <div className="rounded-xl p-3" style={{background:'hsla(270,100%,60%,0.08)',border:'1px solid hsla(270,100%,60%,0.2)'}}>
                    <div className="text-[9px] font-bold uppercase tracking-widest text-purple-400 mb-1 flex items-center gap-1"><Bot size={10} aria-hidden="true" /> AI Commentary</div>
                    <div className="text-xs text-foreground italic" style={{fontFamily:'Rajdhani,sans-serif'}}>"{aiCommentary}"</div>
                  </div>
                )}
                {isElite && currentPlayer && (
                  <button onClick={async()=>{
                    setAiLoading(true);
                    try { const r = await api.post('/packages/ai/analyze',{auctionId:id,playerId:currentPlayer._id,analysisType:'bid_advice'}); setAiData(r.data.analysis); setAiPanel(true); } catch(e){} finally{setAiLoading(false);}
                  }} disabled={aiLoading}
                    className="w-full py-2.5 rounded-xl text-xs font-bold text-purple-300 transition-all disabled:opacity-50"
                    style={{background:'hsla(270,100%,60%,0.1)',border:'1px solid hsla(270,100%,60%,0.25)',fontFamily:'Oswald,sans-serif',letterSpacing:'1px'}}>
                    {aiLoading?<span className="inline-flex items-center gap-1.5"><Loader2 size={12} className="animate-spin" aria-hidden="true" /> ANALYZING...</span>:<span className="inline-flex items-center gap-1.5"><Bot size={12} aria-hidden="true" /> AI BID ADVICE</span>}
                  </button>
                )}
                {aiPanel && aiData && (
                  <div className="rounded-xl p-3 text-xs" style={{background:'hsla(270,100%,60%,0.08)',border:'1px solid hsla(270,100%,60%,0.2)'}}>
                    <div className="flex justify-between mb-2">
                      <span className="text-purple-400 font-bold uppercase inline-flex items-center gap-1" style={{fontSize:"9px"}}><Bot size={10} aria-hidden="true" /> BEAST AI</span>
                      <button onClick={()=>setAiPanel(false)} aria-label="Close" className="text-muted-foreground"><XCircle size={14} aria-hidden="true" /></button>
                    </div>
                    <div className="space-y-1 font-display">
                      <div className="flex justify-between"><span className="text-muted-foreground">Similar sold</span><span>{aiData.similarSold}</span></div>
                      <div className="flex justify-between"><span className="text-muted-foreground">Avg price</span><span className="text-primary">{fmt(aiData.avgSoldPrice)}</span></div>
                      <div className="flex justify-between"><span className="text-muted-foreground">Suggested</span><span className="text-green-400">{fmt(aiData.suggestedMin)}–{fmt(aiData.suggestedMax)}</span></div>
                      <div className="flex justify-between"><span className="text-muted-foreground">Risk</span><span className={aiData.riskLevel==='HIGH'?'text-red-400':aiData.riskLevel==='MEDIUM'?'text-yellow-400':'text-green-400'}>{aiData.riskLevel}</span></div>
                    </div>
                  </div>
                )}
                {isElite && (
                  <a href={`/broadcast/${id}`} target="_blank" rel="noopener noreferrer"
                    className="w-full py-2.5 rounded-xl text-xs font-bold text-center block transition-all"
                    style={{background:'hsla(270,100%,60%,0.1)',border:'1px solid hsla(270,100%,60%,0.25)',color:'hsl(270,100%,75%)',fontFamily:'Oswald,sans-serif',letterSpacing:'1px'}}>
                    <Cast size={12} className="inline mr-1" aria-hidden="true" />OPEN BROADCAST
                  </a>
                )}
              </>
            )}
            ...
            {isTeamOwner&&myTeam&&(
              <>
                {/* Elite features parity — team owners get the same AI bid
                    advice + broadcast access as the organizer whenever the
                    auction itself is running on an Elite package. */}
                {isElite && aiCommentary && (
                  <div className="rounded-xl p-3" style={{background:'hsla(270,100%,60%,0.08)',border:'1px solid hsla(270,100%,60%,0.2)'}}>
                    <div className="text-[9px] font-bold uppercase tracking-widest text-purple-400 mb-1 flex items-center gap-1"><Bot size={10} aria-hidden="true" /> AI Commentary</div>
                    <div className="text-xs text-foreground italic" style={{fontFamily:'Rajdhani,sans-serif'}}>"{aiCommentary}"</div>
                ...
                <div className="rounded-xl p-3" style={{background:`${myTeam.primaryColor}10`,border:`1px solid ${myTeam.primaryColor}30`}}>
                  ...
                  <div className="flex justify-between items-end">
                    ...
                    {rtmEnabled&&<div className="text-right"><div className="font-bold text-sm inline-flex items-center gap-1" style={{color:'hsl(45,100%,51%)',fontFamily:'Oswald,sans-serif'}}><Target size={13} aria-hidden="true" /> {rtmRemaining}</div><div className="text-muted-foreground" style={{fontSize:'10px'}}>RTM</div></div>}
                  </div>
                ...
                {isLeading&&(
                  <div className="w-full py-4 rounded-xl border-2 text-center relative overflow-hidden"
                    ...
                    <div className="font-bold flex items-center gap-1.5" style={{fontFamily:'Oswald,sans-serif',color:myTeam.primaryColor,fontSize:'18px',letterSpacing:'1px'}}><Crown size={16} aria-hidden="true" /> YOU'RE LEADING!</div>
                ...
                {!canBid&&!isLeading&&currentPlayer&&auctionStatus==='active'&&timer>0&&(
                  <div className="text-center py-3">
                    {myTeam.purse<currentBid+bidConfig.bidIncrement
                      ?<p className="text-red-400 text-xs font-bold flex items-center gap-1" style={{fontFamily:'Rajdhani,sans-serif'}}><AlertTriangle size={11} aria-hidden="true" /> Insufficient purse</p>
                ...
                {auctionStatus==='paused'&&(
                  <div className="text-center py-3 rounded-xl" style={{background:'rgba(234,179,8,0.08)',border:'1px solid rgba(234,179,8,0.2)'}}>
                    <p className="text-yellow-400 text-xs font-bold" style={{fontFamily:'Oswald,sans-serif'}}><PauseCircle size={12} className="inline mr-1" />AUCTION PAUSED</p>
                  </div>
                )}
              </>
            )}
            ...
            {isTeamOwner&&!myTeam&&(
              <div className="text-center py-6 rounded-xl" style={{background:'hsla(222,30%,12%,0.5)',border:'1px solid rgba(255,255,255,0.07)'}}>
                <Trophy size={26} className="mx-auto text-primary" style={{marginBottom:'8px'}} aria-hidden="true" />
            ...
            {!isOrganizer&&!isTeamOwner&&(
              <div className="text-center py-4 rounded-xl" style={{background:'hsla(222,30%,12%,0.4)',border:'1px solid rgba(255,255,255,0.05)'}}>
                <p className="text-muted-foreground text-xs flex items-center gap-1" style={{fontFamily:'Rajdhani,sans-serif'}}><Eye size={11} aria-hidden="true" /> Viewing live</p>
              </div>
            )}
          </div>
