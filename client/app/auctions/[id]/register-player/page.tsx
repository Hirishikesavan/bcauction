'use client';
import {
  Camera, CheckCircle, Clock, Lock, CreditCard, Gavel, IndianRupee, TriangleAlert, Upload
} from 'lucide-react';
...
export default function PlayerRegistration() {
  ...
  const [orgPayment,       setOrgPayment]       = useState<any>(null); // organizer UPI/bank info
  ...
  const fetchAuction = async () => {
    try {
      ...
      // Also fetch organizer payment details
      try {
        const p = await api.get(`/packages/organizer-payment/${auctionId}`);
        setOrgPayment(p.data);
      } catch { }
    } catch {
  ...
  // ─── Step 1: Validate form + create Razorpay order OR free register ──────
  const handleProceedToPayment = async (e: React.FormEvent) => {
    ...
    // FREE registration — skip payment entirely
    if (!auction?.registrationFeeEnabled) {
      setLoading(true);
      try {
        const r = await api.post(`/auctions/${auctionId}/players/public-register`, {
          ...form,
    ...
    try {
      ...
      if (r.data.devMode) {
        toast('Dev mode: skipping live payment');
  ...
  const feeDisplay = !auction?.registrationFeeEnabled
    ? 'Free'
  ...
  return (
    <div className="min-h-screen bg-background relative overflow-hidden">
      ...
      <div className="absolute inset-0 pointer-events-none" style={{ background: 'linear-gradient(180deg,hsl(0 0% 0% / 0.5) 0%,hsl(0 0% 0% / 0.85) 100%)' }} />
      ...
      <div className="relative max-w-2xl mx-auto px-4 py-8 sm:py-12">
        ...
        <motion.div initial={{ opacity: 0, y: -16 }} animate={{ opacity: 1, y: 0 }} className="text-center mb-8">
          ...
          <p className="font-display text-foreground text-4xl sm:text-5xl font-bold mt-3 text-gradient-gold">{auction.name}</p>
              ...
              {(['form', 'payment'] as const).map((s, i) => (
                <div key={s} className="flex items-center gap-3">
                  <div className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-heading font-bold transition-all ${
                    ...
                    {step === 'payment' && s === 'form' ? <CheckCircle size={14} /> : i + 1}
                  </div>
        ...
        <AnimatePresence mode="wait">
          ...
          {step === 'success' && registeredPlayer && (
            <motion.div key="success" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0 }}
              ...
              <div className="text-center mb-6">
                ...
                <p className="text-green-400 text-sm font-display mt-1 flex items-center gap-1"><CheckCircle size={14} />Registration complete & saved to organizer dashboard</p>
              </div>
              ...
              <div className="flex items-start gap-5 flex-col sm:flex-row bg-secondary/20 rounded-xl p-5 border border-primary/20">
                <div className="w-24 h-24 rounded-xl overflow-hidden border border-primary/30 flex-shrink-0 bg-secondary/40">
                  {playerPhoto ? (
                    ...
                    <div className="w-full h-full flex items-center justify-center text-4xl">{roleIcons?.[registeredPlayer.role] || 'PLY'}</div>
                  )}
                </div>
              ...
              <div className="flex gap-3 flex-col sm:flex-row mt-6">
                <button type="button"
                  onClick={() => { setStep('form'); setRegisteredPlayer(null); setUploadedImageUrl(null); setPImgPreview(''); setForm({ name:'',role:'Batsman',category:'Gold',nationality:'Indian',age:'24',basePrice:'1000000',battingStyle:'Right Hand',bowlingStyle:'Right Arm',matches:'0',runs:'0',wickets:'0',average:'0',strikeRate:'0' }); }}
          ...
          {step === 'payment' && (
            <motion.div key="payment" initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}
              ...
              <div className="flex justify-center mb-4"><CreditCard size={48} className="text-primary" /></div>
              ...
              <p className="text-xs text-muted-foreground mb-4 flex items-center justify-center gap-1"><Lock size={11} />Powered by Razorpay · 256-bit SSL encryption</p>
              {loading && (
            ...
            <motion.div key="form" initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}
              ...
              <div className="flex items-center gap-3 bg-primary/10 border border-primary/25 rounded-xl px-4 py-3 mb-6">
                ...
                <div>
                  ...
                  <p className="text-foreground text-xs font-display mt-0.5">
                    Registration fee: <span className="text-primary font-semibold">{feeDisplay}</span> · Secure Razorpay checkout · Elite Features Enabled
                  </p>
                </div>
              </div>
              ...
              <form onSubmit={handleProceedToPayment} className="space-y-4">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div>
                    ...
                    <select name="role" value={form.role} onChange={handleInputChange} className={INP} style={{ background: 'hsl(0 0% 8%)' }}>
                      {['Batsman','Bowler','AllRounder','WicketKeeper','Other'].map(r => (
                        <option key={r} value={r} style={{ background: 'hsl(0 0% 8%)' }}>{r}</option>
                      ))}
                    </select>
                  </div>
                  <div>
                    <label className={LBL}>Category *</label>
                    <select name="category" value={form.category} onChange={handleInputChange} className={INP} style={{ background: 'hsl(0 0% 8%)' }}>
                      {['Elite','Gold','Silver','Emerging'].map(c => (
                        <option key={c} value={c} style={{ background: 'hsl(0 0% 8%)' }}>{c}</option>
                      ))}
                    </select>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div>
                    ...
                    <select name="battingStyle" value={form.battingStyle} onChange={handleInputChange} className={INP} style={{ background: 'hsl(0 0% 8%)' }}>
                      <option value="Right Hand" style={{ background: 'hsl(0 0% 8%)' }}>Right Hand</option>
                      <option value="Left Hand" style={{ background: 'hsl(0 0% 8%)' }}>Left Hand</option>
                    </select>
                  </div>
                  <div>
                    <label className={LBL}>Bowling Style</label>
                    <select name="bowlingStyle" value={form.bowlingStyle} onChange={handleInputChange} className={INP} style={{ background: 'hsl(0 0% 8%)' }}>
                      <option value="Right Arm" style={{ background: 'hsl(0 0% 8%)' }}>Right Arm</option>
                      <option value="Left Arm" style={{ background: 'hsl(0 0% 8%)' }}>Left Arm</option>
                    </select>
                  </div>
                </div>
                ...
                <div>
                  <label className={LBL}>
                    ...
                    {uploadedImageUrl && !uploadingImage && <span className="ml-2 text-green-400 flex items-center gap-1"><CheckCircle size={12} />uploaded</span>}
                  </label>
                  <input
                    type="file"
                    accept="image/*"
                  ...
                  {pImgPreview && (
                    <div className="mt-3 flex items-center gap-3">
                      ...
                      <div className="text-xs text-muted-foreground">
                        {uploadedImageUrl ? (
                          <span className="text-green-400 flex items-center gap-1"><CheckCircle size={12} />Photo uploaded to cloud — will appear instantly in organizer dashboard</span>
                        ) : uploadingImage ? (
                          <span className="text-primary animate-pulse flex items-center gap-1"><Upload size={12} />Uploading to cloud…</span>
                        ) : (
                          <span className="text-yellow-400 flex items-center gap-1"><TriangleAlert size={12} />Upload pending</span>
                        )}
                      </div>
                    </div>
                  )}
                ...
                <div className="pt-2">
                  <button
                    ...
                    ) : auction?.registrationFeeEnabled ? (
                      <><CreditCard size={14} className="inline mr-1.5" />Pay {feeDisplay} & Register</>
                    ) : (
                      <><Gavel size={14} className="inline mr-1.5" />Register for Free</>
                    )}
                  </button>

                  {/* Organizer payment info (shown when fee is enabled) */}
                  {auction?.registrationFeeEnabled && orgPayment?.payment && (
                    <div className="mt-4 p-4 rounded-xl border border-primary/20 bg-primary/5">
                      <div className="text-[10px] font-heading uppercase tracking-wider text-muted-foreground mb-2 flex items-center gap-1"><IndianRupee size={9} />Payment goes directly to organizer</div>
                      <div className="space-y-1 text-xs font-display">
                        {orgPayment.payment.upiId && (
                          <div className="flex items-center gap-2">
                            <span className="text-muted-foreground">UPI:</span>
                            <span className="text-foreground font-mono font-bold">{orgPayment.payment.upiId}</span>
                            {orgPayment.payment.upiName && <span className="text-muted-foreground">({orgPayment.payment.upiName})</span>}
                            <button type="button" onClick={() => { navigator.clipboard.writeText(orgPayment.payment.upiId); toast.success('UPI ID copied!'); }}
                              className="ml-auto text-primary text-[10px] font-heading uppercase border border-primary/30 px-2 py-0.5 rounded hover:bg-primary/10 transition-all">Copy</button>
                          </div>
                        )}
                        {orgPayment.payment.whatsapp && (
                          <div className="flex items-center gap-2">
                            <span className="text-muted-foreground">WhatsApp:</span>
                            <span className="text-foreground">{orgPayment.payment.whatsapp}</span>
                          </div>
                        )}
                        {orgPayment.payment.bankName && (
                          <div className="text-muted-foreground">Bank: {orgPayment.payment.bankName} · {orgPayment.payment.ifscCode}</div>
                        )}
                        {orgPayment.payment.qrCodeUrl && (
                          <div className="mt-2">
                            <img src={orgPayment.payment.qrCodeUrl} alt="Payment QR" className="w-24 h-24 rounded-lg border border-primary/20 object-cover" />
                          </div>
                        )}
                      </div>
                    </div>
                  ...
                  <p className="text-center text-xs text-muted-foreground mt-3">
                    {auction?.registrationFeeEnabled
                      ? <><Lock size={11} className="inline mr-1" />Secured by Razorpay · UPI, Cards, Net Banking accepted</>
                      : <><CheckCircle size={11} className="inline mr-1 text-green-400" />Free registration — no payment required</>}
                  </p>
                </div>
              </form>
            </motion.div>
