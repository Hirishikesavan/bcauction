...
const actIcon: Record<string,string> = {
  login_success: 'success', login_failed: 'failed', logout: 'logout',
  register: 'register', verify_email: 'email', password_reset: 'key',
  account_locked: 'locked', account_blocked: 'blocked',
  auction_created: 'auction', auction_started: 'play', auction_completed: 'trophy',
  player_sold: 'sold', bid_placed: 'bid', rtm_used: 'rtm',
  team_joined: 'team', admin_action: 'admin',
};
...
export default function AdminPanel() {
  ...
  useEffect(() => {
    const check = async () => {
      try {
        // Use authClient.getSession() — works with Better Auth session cookies
        const session = await authClient.getSession();
        const u = (session?.data?.user) as any;
        const adminEmail = ADMIN_EMAIL.toLowerCase();
        if (!u || (u.role !== 'admin' && u.email?.toLowerCase() !== adminEmail)) {
  ...
  const deleteAuction = async (id: string, name: string) => {
    ...
    try {
      ...
      showToast(`Deleted auction: ${name}`);
      fetchStats();
    } catch (e:any) { showToast('Failed'); }
  };
  const logout = () => {
            ...
            <div style={S.card}>
              ...
              {users.map(u => (
                <div key={u._id} style={{ ...S.row }}>
                  ...
                  <span style={{ flex:1, color:u.isVerified?'#10b981':'#ef4444', fontSize:11 }}>{u.isVerified?'Yes':'No No'}</span>
