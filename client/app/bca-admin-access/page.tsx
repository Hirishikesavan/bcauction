...
export default function AdminAccessPage() {
  ...
  const onSubmit = async (d: F) => {
    ...
    try {
      ...
      const res = await authClient.signIn.email({ email: emailClean, password: d.password });
      if (res.error) {
      ...
      const adminEmail = (process.env.NEXT_PUBLIC_ADMIN_EMAIL || 'hirishi2020@gmail.com').toLowerCase();
      if (user?.role === 'admin' || emailClean === adminEmail) {
