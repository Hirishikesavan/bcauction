'use client';
import {
  Gavel, BarChart3, Radio, Trophy, UserRound, Calendar, IndianRupee as IR,
  TrendingUp, CheckCircle, Clock, Target, Users, Crown
} from 'lucide-react';
...
export default function ViewerDashboard() {
  ...
  return (
    <div className="min-h-screen bg-background relative">
        ...
        <div style={{position:'fixed',inset:0,background:'linear-gradient(180deg,hsl(0 0% 0% / 0.55) 0%,hsl(0 0% 0% / 0.75) 100%)',pointerEvents:'none',zIndex:0}}/>
        ...
        <div className="max-w-7xl mx-auto px-6 py-8 relative z-10">
          ...
          {loading ? (
            ...
            <div className="text-center py-24 bg-glass-premium rounded-xl border-gold-subtle">
              <div className="flex justify-center mb-4"><Gavel size={48} className="text-primary" /></div>
            ...
            <>
              ...
              {tab === 'overview' && (
                <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }}>
                      ...
                      <div className="flex flex-col items-end gap-2">
                        ...
                        {selAuction.status === 'active' && (
                          <Link href={`/auctions/${selAuction._id}`}
                            ...
                            <Radio size={12} className="inline mr-1.5 text-red-300" />Watch Live
                          </Link>
                        )}
                        <Link href="/reports"
                          className="px-4 py-2 rounded-lg border border-blue-500/30 text-blue-400 text-xs font-heading uppercase tracking-wider hover:bg-blue-500/10 transition-all">
                          <BarChart3 size={12} className="inline mr-1.5" />Reports
                        </Link>
                      </div>
                    </div>
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                      {[
                        [<Calendar size={12} />, 'Date',         selAuction.date ? format(new Date(selAuction.date), 'dd MMM yyyy') : '—'],
                        [<Clock size={12} />,    'Bid Timer',    `${selAuction.bidTimer}s`],
                        [<IR size={12} />, 'Purse/Team',   fmt(selAuction.totalPursePerTeam)],
                        [<TrendingUp size={12} />, 'Bid Increment', fmt(selAuction.bidIncrement)],
                      ].map(([ic, l, v]) => (
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
                    {[
                      [<Trophy size={14} />, teams.length,        'Teams'],
                      [<UserRound size={14} />, players.length,       'Players'],
                      [<CheckCircle size={14} className="text-green-400" />, soldPlayers.length,   'Sold'],
                      [<Clock size={14} />, unsoldPlayers.length, 'Remaining'],
                    ].map(([ic, v, l]) => (
                ...
                <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }}>
                  {selAuction.status !== 'active' ? (
                    <div className="text-center py-20 bg-glass-premium rounded-xl border-gold-subtle">
                      <div className="flex justify-center mb-4">{selAuction.status === 'completed' ? <Trophy size={48} className="text-yellow-400" /> : <Clock size={48} className="text-primary" />}</div>
                      ...
                      <div className="lg:col-span-2">
                        <div className="bg-glass-premium rounded-xl p-6 border-gold-subtle mb-4">
                          <div className="flex items-center justify-between mb-4">
                            ...
                            <Link href={`/auctions/${selAuction._id}`}
                              className="px-4 py-2 rounded-lg bg-primary text-primary-foreground text-xs font-heading uppercase tracking-wider glow-gold hover:scale-[1.02] transition-all">
                              <Radio size={12} className="inline mr-1.5 text-red-300" />Join Live Room
                            </Link>
                          </div>
                          {liveState?.currentPlayer ? (
                            <div className="flex gap-5 items-start">
                              <div className="relative rounded-xl overflow-hidden flex-shrink-0"
                                style={{ width: 120, height: 160, background: 'hsl(0 0% 5%)' }}>
                                {liveState.currentPlayer.imageUrl
                                  ...
                                  : <div className="w-full h-full flex items-center justify-center text-4xl">{roleIcons[liveState.currentPlayer.role] || 'PLY'}</div>}
                              ...
                              <div className="flex-1">
                                ...
                                <div className="bg-glass-navy rounded-lg p-4 mb-3">
                                  ...
                                  {liveState.leadingTeamName && (
                                    <div className="text-sm font-display text-foreground mt-1 flex items-center gap-1"><Crown size={13} className="text-yellow-400" />{liveState.leadingTeamName}</div>
                                  )}
                                </div>
                                {/* Timer */}
              ...
              {tab === 'teams' && (
                <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }}>
                  {teams.length === 0 ? (
                    <div className="text-center py-20 bg-glass-premium rounded-xl border-gold-subtle">
                      <div className="flex justify-center mb-4"><Trophy size={48} className="text-primary" /></div>
                        ...
                        <div key={team._id} className="bg-glass-premium rounded-xl overflow-hidden border-gold-subtle hover:border-gold transition-all">
                          ...
                          <div className="p-5">
                            ...
                            <div className="grid grid-cols-3 gap-2 mb-3">
                              {[
                                [<IR size={11} />, fmt(team.purse), 'Purse'],
                                [<Users size={11} />, `${team.playersCount}/${team.maxPlayers}`, 'Players'],
                                [<Target size={11} />, `${(team.rtmTotal || 0) - (team.rtmUsed || 0)}`, 'RTM'],
                              ].map(([ic, v, l]) => (
              ...
              {tab === 'players' && (
                <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }}>
                  ...
                  {players.length === 0 ? (
                    <div className="text-center py-20 bg-glass-premium rounded-xl border-gold-subtle">
                      <div className="flex justify-center mb-4"><UserRound size={48} className="text-primary" /></div>
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-3">
                      {players.map(p => (
                        <div key={p._id} className="bg-glass-premium rounded-xl overflow-hidden border-gold-subtle hover:border-gold transition-all">
                          <div className="relative overflow-hidden" style={{ height: 140, background: 'hsl(0 0% 5%)' }}>
                            {p.imageUrl
                              ? <img src={imgUrl(p.imageUrl)} alt={p.name} className="w-full h-full object-cover object-top"/>
                              : <div className="w-full h-full flex items-center justify-center text-4xl">{roleIcons[p.role] || 'PLY'}</div>}
