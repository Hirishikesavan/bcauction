...
// NOTE: This legacy auth file is NOT mounted — Better Auth handles /api/auth/*
// Kept only for reference. Do not add JWT imports here.
...
router.post('/create-role-session', async (req, res) => {
  try {
    const { role } = req.body;
...
router.post('/login', async (req, res) => {
  try {
    ...
    const accessToken  = 'legacy-not-used';
    const refreshToken = 'legacy-not-used';
    ...
    return res.json({ success: true, user: updatedUser.toPublic(), token: 'use-better-auth' });
